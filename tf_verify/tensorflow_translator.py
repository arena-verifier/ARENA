...
class TFTranslator:
	...
	def __init__(self, model, session = None):
		...
		# tf.saved_model.save(model, "/home/zhongyy/Verifier_nn/ERAN/tf_verify/res.tf")
	...
	def staging_trans(self, stage_layer_num, stage_block_num):
		"""
		The constructor has produced a graph_def with the help of the functions graph_util.convert_variables_to_constants and graph_util.remove_training_nodes.
		translate() takes that graph_def, imports it, and translates it into two lists which then can be processed by an Optimzer object.
		...
		reshape_map = {}
		operations_to_be_ignored = ["Reshape", "Pack", "Shape", "StridedSlice", "Prod", "ConcatV2"]
		operations_to_be_ignored_without_reshape = ["NoOp", "Assign", "Const", "RestoreV2", "SaveV2",
													"IsVariableInitialized", "Identity"]
			...
			with tf.Session() as sess:
				...
				for op in graph.get_operations():
					...
					if op.type == "MatMul":
						...
						deepzono_res = deeppoly_res
						operation_resources.append({'deepzono': deepzono_res, 'deeppoly': deeppoly_res})
						assert stage_block_num or stage_layer_num
						if (stage_layer_num):
							layer_counter = layer_counter + 1
							if (layer_counter == stage_layer_num):
								assert len(operation_types) == len(operation_resources)
								operations_list.append(operation_types)
								resources_list.append(operation_resources)
								operation_types = ["Placeholder"]
								in_out_placeholder = ([], op.outputs[0].name, tensorshape_to_intlist(op.outputs[0].shape))
					...
					#	operation_resources.append({'deepzono':deepzono_res, 'deeppoly':deeppoly_res})
						...
						# print("operation type1 ",in_out_info,op.inputs[0].shape,op.inputs[1].shape)
						assert 0, "Operations of type " + op.type + " in " + str(
							[o.type for o in graph.get_operations()]) + " are not yet supported."
				if(len(operation_types)>1):
					operations_list.append(operation_types)
					resources_list.append(operation_resources)
				assert len(operations_list) == len(resources_list)
				#for i in range(len(operations_list)):
					#print ("The elemnt ", i)
					#print (operations_list[i])
				return operations_list,resources_list
...
if __name__ == '__main__':
	tf_file = '/home/zhongyy/Verifier_nn/ERAN/test_nns/cifar_nns/ResNet18_DiffAI.pyt'
	model, is_conv, means, stds = read_tensorflow_net(tf_file, 3072, True)
	# tf.saved_model.save(model, "/home/zhongyy/Verifier_nn/ERAN/tf_verify")
