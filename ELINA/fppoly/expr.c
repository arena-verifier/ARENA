...
expr_t * expr_replace_bounds_affine(fppoly_internal_t *pr, expr_t * expr, neuron_t ** neurons, bool is_lower){
	...
	//Independently handle the first coefficient in the expression
	if(mul_expr==NULL){
    ...
    //Handle rest of the coefficients
...
expr_t * expr_replace_bounds_activation(fppoly_internal_t * pr, expr_t * expr, neuron_t ** neurons, bool is_lower){
	...
	for(i = 0; i < num_neurons; i++){
		...
		if(expr->sup_coeff[i]<0 || expr->inf_coeff[i] < 0){
			//If the variable restrictly takes non-zero coeff, do the replacment
			...
			// lambda is the replaced symbolic constraint, expr is the coefficient 
			...
			// pr->min_denormal is the smallest positive number that floating-point variable can be assigned
		}
		else{
			// Else if one of inp or suf coeff is 0, therefore the coeff in the result expressoin is also 0
			res->inf_coeff[i] = 0.0;
			res->sup_coeff[i] = 0.0;
			double tmp1, tmp2;
...
void printf_expr(expr_t * source){
	size_t num_neurons; 
